...
public class MSSUpdatePreFlagTasklet implements Tasklet {
	/**
	 * LOGGER
	 */
	final static Logger LOGGER = LoggerFactory.getLogger(MSSUpdatePreFlagTasklet.class);
	...
	/**
     * This method will update the MSS_TRASFER_CD to "Y".
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		try {
			...
			if(!CommonUtil.isNull(beginDate)){
				int rowsUpdated = omniJdbcTemplate.update(query, new Object[]{endDate,beginDate});
				LOGGER.debug("Number of rows updated with MSS_TRANSFER_CD = 'I' :- "+rowsUpdated);
			} else {
				int rowsUpdated = omniJdbcTemplate.update(query, new Object[]{endDate});
				LOGGER.debug("Number of rows updated with MSS_TRANSFER_CD = 'I' :- "+rowsUpdated);
			}
			
		} catch (NullPointerException e) {
			LOGGER.error(" Error occoured at execute method of MSSUpdatePreFlagTasklet - ", e);
			...
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug(" Exiting execute method of MSSUpdatePreFlagTasklet ");
			}
		}
