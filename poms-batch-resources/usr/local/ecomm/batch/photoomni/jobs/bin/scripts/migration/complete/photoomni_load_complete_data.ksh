...
COMPLE_ZIP_DIR=/usr/local/ecomm/data/photoomni/ftp/migration/complete
flatFileDir=/usr/local/ecomm/data/photoomni/work/migration/complete
PLSQL_FILE_PATH=/usr/local/ecomm/batch/photoomni/jobs/bin/scripts/dbscripts/migration
ARCHIVE=/usr/local/ecomm/data/photoomni/admin/archive/migration/complete
...
<<`
sqlplus -S $DBLOG/$DBPWD << SQL
insert into Om_transfer_job_status values($jobId,'BATCH - PHOTO OMNI COMPLETE DATA LOAD','W',sysdate,'JOB START(PHOTO OMNI DATA LOAD)',sysdate,sysdate,'photo',sysdate,'photo',sysdate);
SQL

#####################EXTRACT ZIP FILES####################

for f in `ls $COMPLE_ZIP_DIR/*.zip -rt`
do
